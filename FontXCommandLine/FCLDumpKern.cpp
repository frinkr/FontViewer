    ...
    private:
        void dumpKern(FXPtr<FXFace> face) {
            ...
            for (auto [kern, count]: kernMap) {
                FXString s;
                if (count < 2)
                    s = FXUnicode::utf8Str(0x258F);// "▏";
                else if (count < 3)
                    s = FXUnicode::utf8Str(0x258E); //"▎";
                else if (count < 4)
                    s = FXUnicode::utf8Str(0x258D); //"▍";
                else if (count < 5)
                    s = FXUnicode::utf8Str(0x258C); //"▌";
                else if (count < 6)
                    s = FXUnicode::utf8Str(0x258B); //"▋";
                else if (count < 7)
                    s = FXUnicode::utf8Str(0x258A); //"▊";
                else if (count < 8)
                    s = FXUnicode::utf8Str(0x2589); //"▉";
                else if (count < 9)
                    s = FXUnicode::utf8Str(0x2588); //"█";
                else if (count < 10)
                    s = FXUnicode::utf8Str(0x2587); //"▇";
                else
                    for (size_t i = 0; i < int(count / 10); ++i)
                        s += FXUnicode::utf8Str(0x2586);// "▆";
